...
--------------------------------
This document describes configuration steps for configuring an AGC SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure SkillPort:
...
SkillPort Configuration in AGC Workflow
----------------------------------------------
...
SkillPort, select
SkillPort and click on **Add** button
...
To configure SkillPort provide following inputs:
	- **RelayState** : The relay state can be an absolute path, such as /hr/index.html; it can be a URL, such as https://www.abc.com/index.html; or, it can be anything that the service provider understands.
...
#. Identify the Saml SSO object created for for SkillPort and export SAML Metadata.

You can use the exported IdP SAML Metadata to configure the IdP Provider configuration in SkillPort service.
