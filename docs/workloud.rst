...
--------------------------------
This document describes configuration steps for configuring an AGC SAML Identity Provider for SaaS Application workflow using a SaaS Application template. Follow the steps below to configure workloud:
...
workloud Configuration in AGC Workflow
----------------------------------------------
...
workloud, select
workloud and click on **Add** button
...
To configure workloud provide following inputs:
	- **Oraganization ID** : Specify organization ID which is sent to workloud service in SAML attribute Organization_ID
	- **Customer ID** : Specify customer ID which is used in entityID and ACS URIs
...
#. Identify the Saml SSO object created for for workloud and export SAML Metadata.

You can use the exported IdP SAML Metadata to configure the IdP Provider configuration in workloud service.
